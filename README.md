...
Tool annotations are client-facing risk hints, not authorization or runtime guarantees. Clients can use trusted hints to adjust confirmation, retry, and trust-boundary UX, but must treat annotations from untrusted servers as untrusted. See [Tool Annotations as Risk Vocabulary](https://blog.modelcontextprotocol.io/posts/2026-03-16-tool-annotations/) for the MCP project's guidance.
...
| `read_only` | optional | bool | *unset* | Emits `annotations.readOnlyHint` verbatim when set (explicit presence; unset emits nothing and clients apply the MCP default, false). A trusted `read_only: true` hint can let a client reduce confirmation friction. An RPC (or final tool name, after `tool_prefix` / `name` override) that starts with a mutating verb (`Create`/`Update`/`Delete`/`Replace`/`Apply`/…) may not carry `read_only: true` — codegen fails. The check is a word-boundary heuristic: it catches `DeleteWidget` / `delete_widget`, not concatenations like `deletewidget`, and it cannot know semantics — a genuinely read-only `SetDifference` is a false positive. Escape hatches: per-RPC `disable_read_only_name_lint: true`, or the global plugin opt `read_only_name_lint=false`. |
| `idempotent` | optional | bool | *unset* | Emits `annotations.idempotentHint` verbatim when set. `idempotent: true` tells the client retry is safe. Unset emits nothing (MCP default: false). |
| `destructive` | optional | bool | *unset* | Emits `annotations.destructiveHint` verbatim when set. The MCP default when absent is **true** (assume destructive), so `destructive: false` is meaningful: it asserts a non-read-only tool performs only additive updates (e.g. a create). Clients may confirm before calling destructive tools and ignore this hint when `read_only` is true. |
| `open_world` | optional | bool | *unset* | Emits `annotations.openWorldHint` verbatim when set. The MCP default when absent is **true**, so `open_world: false` is meaningful: it asserts the tool stays within the server's own domain. `open_world: true` signals a deployment-specific trust-boundary crossing (web fetch, third-party API, search) whose output may carry untrusted content, so clients can adjust pre- and post-call UX. |
...
| `exclude_from_outputs` | *empty* | Fully-qualified RPC names (`package.Service.Method`) whose **tool output** omits this field, from both the generated output schema and the serialized response (`MarshalProtoMaskedFor`). Every other RPC keeps the field, so a list RPC can drop a heavy repeated field that the matching detail RPC still returns; `exclude` would hide it from both. Applies to outputs only: input schemas and request unmarshaling ignore it. Each name must resolve, within one generation run, to an RPC that carries `protomcp.v1.tool`, otherwise generation fails, since the proto compiler never checks these strings itself. A run sees the files it generates plus their imports, so keep the field in the same file as the RPC, or generate the module in a single run (buf: `strategy: all`) when the message lives in a file the service imports; the error names both options. Removal happens on schema and payload together, so the tool never advertises a shape its responses lack, and it composes with `exclude` rather than replacing it. Say so in the list tool's `description` and point at the detail tool, or the model cannot learn the data exists elsewhere. |
