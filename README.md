...
| `read_only` | optional | bool | *unset* | Emits `annotations.readOnlyHint` verbatim when set (explicit presence; unset emits nothing and clients apply the MCP default, false). `read_only: true` hints the LLM the tool is safe to call speculatively. An RPC (or final tool name, after `tool_prefix` / `name` override) that starts with a mutating verb (`Create`/`Update`/`Delete`/`Replace`/`Apply`/…) may not carry `read_only: true` — codegen fails. The check is a word-boundary heuristic: it catches `DeleteWidget` / `delete_widget`, not concatenations like `deletewidget`, and it cannot know semantics — a genuinely read-only `SetDifference` is a false positive. Escape hatches: per-RPC `disable_read_only_name_lint: true`, or the global plugin opt `read_only_name_lint=false`. |
