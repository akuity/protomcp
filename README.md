...
| `exclude_from_outputs` | *empty* | Fully-qualified RPC names (`package.Service.Method`) whose **tool output** omits this field, from both the generated output schema and the serialized response (`MarshalProtoMaskedFor`). Every other RPC keeps the field, so a list RPC can drop a heavy repeated field that the matching detail RPC still returns; `exclude` would hide it from both. Applies to outputs only: input schemas and request unmarshaling ignore it. Each name must resolve to an RPC in the generation set that carries `protomcp.v1.tool`, otherwise generation fails, since the proto compiler never checks these strings itself. Removal happens on schema and payload together, so the tool never advertises a shape its responses lack, and it composes with `exclude` rather than replacing it. Say so in the list tool's `description` and point at the detail tool, or the model cannot learn the data exists elsewhere. |
